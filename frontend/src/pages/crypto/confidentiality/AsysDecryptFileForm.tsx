...
type AsysFileDecryptFormValues = z.infer<typeof schema>;

export default function AsysFileDecryptForm() {
    ...
    const form = useForm<AsysFileDecryptFormValues>({
    ...
    const onSubmit = async (data: AsysFileDecryptFormValues) => {
        ...
        try {
            ...
            const res = await CryptoService.decryptAsymmetricFile(
                data.privateKeyRef,
                data.inputFile,
                algo,
                padding
        ...
        >
            <Card className="p-6 bg-[#fcfcfc] border border-[#c7c7cc] rounded shadow-[0_0_3px_1px_rgba(0,0,0,0.2)] space-y-6">
                <h2 className="text-2xl font-bold text-gray-800 flex items-center m-0">
                    📁 Déchiffrement de fichier asymétrique
                </h2>
