...
type FileEncryptFormValues = z.infer<typeof schema>;

export default function FileEncryptForm() {
  const [keys, setKeys] = useState<KeyMaterial[]>([]);
  const [result, setResult] = useState<FileCryptoResponseDTO | null>(null);

  const form = useForm<FileEncryptFormValues>({
  ...
  // ==============================
  const onSubmit = async (data: FileEncryptFormValues) => {
    try {
      const res = await CryptoService.encryptSymmetricFile(
        data.keyId,
        data.inputFile,
        data.mode
      );
    ...
    >
      <Card className="p-6 bg-[#fcfcfc] border border-[#c7c7cc] rounded shadow-[0_0_3px_1px_rgba(0,0,0,0.2)] space-y-6">
        <h2 className="text-2xl font-bold text-gray-800 flex items-center m-0">
          📁 Chiffrement de fichier symétrique
        </h2>
