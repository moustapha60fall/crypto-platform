...
const schema = z.object({
    wrappingKeyRef: z.string().min(1, "Clé d’enveloppement (AES) requise"),
    targetKeyRef: z.string().min(1, "Clé cible (AES) requise"),
...
// Composant principal
// ====================================
export default function WrappingSysSysForm() {
    ...
    useEffect(() => {
        ...
        const pad = form.watch("padding");

        try {
            validateAlgorithmPaddingCompatibility(algo, pad);
        } catch {
            form.setValue("padding", PaddingTypes.PKCS5Padding);
        }
    }, [form.watch("algorithm"), form.watch("padding")]);
    ...
    const onSubmit = async (data: FormValues) => {
        ...
        try {
            ...
            toast.success("Clé AES enveloppée avec succès (AES → AES)");
        } catch (err: any) {
            console.error(err);
        ...
        >
            <Card className="p-6 bg-[#fcfcfc] border border-[#c7c7cc] rounded shadow-[0_0_3px_1px_rgba(0,0,0,0.2)] space-y-6">
              ...
              rounded-[3px] hover:shadow-[0_0_3px_1px] transition">
                                    <FormLabel>Clé symétrique à envelopper</FormLabel>
                                    <FormControl>
                                        <Select onValueChange={field.onChange} value={field.value}>
                                            <SelectTrigger>
                                                <SelectValue placeholder="Sélectionner la clé cible" />
                                            </SelectTrigger>
                                            <SelectContent>
                                                {symmetricKeys.map((k) => (
              ...
              border border-[#c7c7cc] dark:border-gray-700
              rounded-[3px] hover:shadow-[0_0_3px_1px] transition">
                                    <FormLabel>Clé d’enveloppement (AES maître)</FormLabel>
                                    <FormControl>
                                        <Select onValueChange={field.onChange} value={field.value}>
                                            <SelectTrigger>
                                                <SelectValue placeholder="Sélectionner la clé d’enveloppement" />
                                            </SelectTrigger>
                                            <SelectContent>
                                                {symmetricKeys.map((k) => (
                                                    <SelectItem key={k.keyRef} value={k.keyRef} className="focus:shadow-[0_0_3px_1px]">
