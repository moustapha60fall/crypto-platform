...
const schema = z.object({
  ...
  inputData: z.string().min(1, "Texte chiffré requis"),
});

type FormValues = z.infer<typeof schema>;

export default function AsysDecryptForm() {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      const result = await CryptoService.decryptAsymmetric(dto);
      setOutput(result.outputData ?? null);
      toast.success("Déchiffrement réussi");
    } catch (err: any) {
      console.error(err);
      toast.error(err?.response?.data ?? "Erreur de déchiffrement");
    }
  };
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <FormField
              ...
              border border-[#c7c7cc] dark:border-gray-700 
              rounded-[3px] hover:shadow-[0_0_3px_1px] transition">
                  <FormLabel>Données chiffrées</FormLabel>
                  <FormControl>
                    <Textarea
                      {...field}
                      placeholder="Texte chiffré en base64"
                      inputMode="text"
                      autoComplete="off"
