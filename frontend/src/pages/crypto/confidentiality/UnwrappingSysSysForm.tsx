...
// 🔓 Composant principal
// ====================================
export default function UnwrappingSysSysForm() {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      toast.success("✅ Clé AES restaurée avec succès");
    } catch (err: any) {
      console.error(err);
    ...
    >
      <Card className="p-6 bg-[#fcfcfc] border border-[#c7c7cc] rounded shadow-[0_0_3px_1px_rgba(0,0,0,0.2)] space-y-6">
        <h2 className="text-2xl font-bold text-gray-800 flex items-center m-0">
          🔓 Désenveloppement Sym → Sym (AES → AES)
        </h2>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <FormField
              ...
              border border-[#c7c7cc] dark:border-gray-700
              rounded-[3px] hover:shadow-[0_0_3px_1px] transition">
                  <FormLabel>Clé AES enveloppée (base64)</FormLabel>
                  <FormControl>
                    <Textarea
                      {...field}
                      placeholder="Coller ici la clé AES enveloppée"
                      rows={6}
        ...
        {unwrappedKeyBase64 && (
          ...
          >
            <strong>Clé AES restaurée :</strong>
